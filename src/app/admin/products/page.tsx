...
const AdminProducts = () => {
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
        <div className="p-4 border-b">
          <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
            <div className="text-lg font-medium text-gray-700">Total Products: {filteredProducts.length}</div>
              ...
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-medium truncate max-w-[200px]">
                      {product.title}
                    </td>
      ...
      {/* Product Modal */}
      {isModalOpen && <ProductModal isOpen={isModalOpen} onClose={handleModalClose} product={productToEdit as any} />}
    </div>
  );
};
