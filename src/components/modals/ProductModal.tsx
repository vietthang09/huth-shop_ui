...
export default function ProductModal({
    ...
    const [attributePrices, setAttributePrices] = useState<Record<number, {
        netPrice: number | null,
        retailPrice: number | null,
        discount: number | null
    }>>({}); useEffect(() => {
        const fetchAttributes = async () => {
            setLoadingAttributes(true); try {
                ...
                if (response.success) {
                    ...
                    if (product && product.properties && product.properties.length > 0) {
                        ...
                        const attributePricesData: Record<number, {
                            netPrice: number | null,
                            retailPrice: number | null,
                            discount: number | null
                        }> = {};
    ...
    const handleAttributePriceChange = (attributeId: number, priceType: 'netPrice' | 'retailPrice' | 'discount', value: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = isEditMode
                ...
                : await addProduct(productData); if (response.success) {
                ...
                setError(response.error ?
                    (Array.isArray(response.error) ? response.error.map(e => e.message).join(', ') : response.error)
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">            <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="mt-6 border-t pt-4">
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {attributes.map((attribute) => (
                                            <tr key={attribute.id} className={`hover:bg-gray-50 ${selectedAttributes.has(attribute.id) ? 'bg-blue-50' : ''}`}>
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{attribute.unit || '-'}</td>                                                {/* Net Price Input */}
                                                    ...
                                                    {selectedAttributes.has(attribute.id) && (
                                                        <div className="flex items-center">
                                                            ...
                                                            <input type="number"
                                                                min="0"
                                                                step="0.01"
                                                                value={attributePrices[attribute.id]?.netPrice !== null ? attributePrices[attribute.id]?.netPrice : attribute.netPrice}
                                                    ...
                                                    {selectedAttributes.has(attribute.id) && (
                                                        <div className="flex items-center">
                                                            ...
                                                            <input type="number"
                                                                min="0"
                                                                step="0.01"
                                                                value={attributePrices[attribute.id]?.retailPrice !== null ? attributePrices[attribute.id]?.retailPrice : attribute.retailPrice}
                                                                onChange={(e) => handleAttributePriceChange(attribute.id, 'retailPrice', e.target.value)}
                                                                className="w-20 px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                                                placeholder="0.00"
                                                            />
                                                        </div>
                                                    ...
                                                    {selectedAttributes.has(attribute.id) && (
                                                        <div className="flex items-center">
                                                            ...
                                                            <input type="number"
                                                                ...
                                                                max="100"
                                                                step="1"
                                                                value={attributePrices[attribute.id]?.discount !== null ? attributePrices[attribute.id]?.discount : attribute.discount}
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500 truncate max-w-[120px]" title={attribute.propertiesHash}>
                        ...
                        {attributes.length > 0 && (
                            <div className="mt-3 text-sm text-gray-600">
                                Selected attributes: <span className="font-medium">{selectedAttributes.size}</span> of {attributes.length}
                            </div>
                        )}
                    </div>
